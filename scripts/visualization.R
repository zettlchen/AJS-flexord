...
algo_levels <- c("kmeans", "kmodes", "kmedians", "kgower", "kgowerMed", "kGDM2centOpt",
                 "kGDM2centMode", "PAM:Gower", "PAM:GDM2",
                 "regularized normal", "multinomial", "binomial", "betabinomial")
algo_labels <- c("kmeans", "kmodes", "kmedians", "kGowercentMin", "kGowercentMedian", "kGDM2centOpt",
                 "kGDM2centMode", "PAM:Gower", "PAM:GDM2",
...
sim = Sys.glob("../data/sim_backpain_sample_size_ncat_*_alpha*.rds") %>%
  ...
  setNames(gsub('\\D|2', '', names(.))) %>% #extracting info an alphas (2 is necessary for GDM*2*)
  bind_rows(.id='alpha') |> 
  mutate(algo=gsub('om$', 'omial', algo),
         algo=gsub('regnorm', 'regularized normal', algo)) |> #renaming algos
  mutate(alpha=as.factor(as.numeric(alpha)),
         ...
         type = case_when(algo %in% c('betabinomial', 'binomial', 'multinomial',
                                      'regularized normal') ~ 'model-based',
                          TRUE ~ 'partitioning'),
         type = factor(type, levels = c('partitioning', 'model-based'))) |>
